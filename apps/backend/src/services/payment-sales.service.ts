...
import { getFallbackPaymentStats, getFallbackPayments, getFallbackPaymentsByClient, getFallbackPaymentsByInvoice, isDatabaseUnavailableError } from './dev-fallback-data.service'
...
export class PaymentService {
  ...
  static async getPaymentById(id: string, ownerScopeId: string): Promise<any | null> {
    try {
      const payment = await prisma.payment.findFirst({ where: { id, userId: ownerScopeId }, include: { invoice: { include: { client: true } }, user: true } })
      return payment ? mapPayment(payment) : null
    } catch (error) {
      if (isDatabaseUnavailableError(error)) {
        return getFallbackPayments(ownerScopeId, { }, { page: 1, limit: 1000 }).data.find((payment) => payment.id === id) || null
      }
      throw error
    }
  }
  static async getPayments(ownerScopeId: string, pagination: PaginationParams, filters: PaymentFilters = {}): Promise<PaginationResponse<any>> {
    try {
      ...
      if (isDatabaseUnavailableError(error)) {
        return getFallbackPayments(ownerScopeId, filters, pagination)
      }
      throw error
    }
  }
  static async getPaymentsByInvoice(invoiceId: string, ownerScopeId: string): Promise<any[]> {
    try {
      const rows = await prisma.payment.findMany({ where: { invoiceId, userId: ownerScopeId }, include: { invoice: { include: { client: true } }, user: true }, orderBy: { paymentDate: 'desc' } })
      return rows.map(mapPayment)
    } catch (error) {
      if (isDatabaseUnavailableError(error)) {
        return getFallbackPaymentsByInvoice(ownerScopeId, invoiceId)
      }
      throw error
    }
  }
  static async getPaymentsByClient(clientId: string, ownerScopeId: string): Promise<any[]> {
    try {
      const rows = await prisma.payment.findMany({ where: { userId: ownerScopeId, invoice: { clientId } }, include: { invoice: { include: { client: true } }, user: true }, orderBy: { paymentDate: 'desc' } })
      return rows.map(mapPayment)
    } catch (error) {
      if (isDatabaseUnavailableError(error)) {
        return getFallbackPaymentsByClient(ownerScopeId, clientId)
      }
      throw error
    }
  }
  ...
  static async getPaymentStats(ownerScopeId: string) {
    try {
      ...
      if (isDatabaseUnavailableError(error)) {
        return getFallbackPaymentStats(ownerScopeId)
      }
      throw error
    }
  }
