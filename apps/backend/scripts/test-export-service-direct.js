...
// Utilisation d'import dynamique pour les modules TypeScript
let ExportService, ClientService, OrderService, prisma

async function loadModules() {
  const exportModule = await import('../src/services/export.service.js')
  const clientModule = await import('../src/services/client.service.js')
  const orderModule = await import('../src/services/order.service.js')
...
async function testExportServiceDirect() {
  ...
  try {
    ...
    console.log('\n5. 📊 Test export Excel commandes...')
    try {
      const excelBuffer = await ExportService.generateOrdersExcel(orders)
    ...
    try {
      const company = await prisma.company.findUnique({ where: { id: companyId } })
      const pdfBuffer = await ExportService.generateOrdersPdf(orders, company)
