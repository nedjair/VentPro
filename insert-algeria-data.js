...
async function insertAlgerianData() {
  try {
    console.log('📝 Insertion des données algériennes...');
    
    // Vérifier si des données existent déjà
    const clientCount = await pool.query('SELECT COUNT(*) FROM clients');
    if (parseInt(clientCount.rows[0].count) > 0) {
      console.log('⚠️ Des données existent déjà. Suppression...');
    ...
    // Insérer des clients de test algériens
    const testClients = [
    ...
    // Insérer des produits de test algériens (prix en DZD)
    const testProducts = [
      // Électronique
    ...
    console.log(`📊 ${testClients.length} clients et ${testProducts.length} produits ajoutés`);
